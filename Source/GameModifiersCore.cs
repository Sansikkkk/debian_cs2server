...
public class GameModifiersCore : BasePlugin, IPluginConfig<GameModifiersConfig>
{
    ...
    public void OnAddModifier(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            GameModifiersUtils.PrintTitleToChat(player, $"Modifier {modifierName} adăugat.");
        }
        else
        {
    ...
    public void OnAddRandomModifier(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            GameModifiersUtils.PrintTitleToChat(player, "Eroare la adăugarea modifierului random.");
        }
    }
    ...
    public void OnAddRandomModifiers(CCSPlayerController? player, CommandInfo info)
    {
        ...
        if (int.TryParse(info.GetArg(1), out int modifierCount))
        {
            if (AddRandomModifiers(modifierCount, out List<GameModifierBase> addedModifiers))
            {
                ...
                {
                    GameModifiersUtils.PrintTitleToChat(player, $"Se adaugă {modifierCount.ToString()} modifiere random.");
                }
                else
                {
                    GameModifiersUtils.PrintTitleToChat(player, $"S-au adăugat doar {addedModifiers.Count.ToString()} modifiere random.");
                }
        ...
        GameModifiersUtils.PrintTitleToChat(player, "Eroare la adăugarea modifierelor random.");
    }
    ...
    public void OnRemoveModifiers(CCSPlayerController? player, CommandInfo info)
    {
        ...
        GameModifiersUtils.PrintTitleToChat(player, "Toate modifierele au fost eliminate.");
    }
    ...
    public void OnRandomRounds(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            GameModifiersUtils.PrintTitleToChat(player, "Nu există modifiere înregistrate! Random rounds nu poate fi activat!");
            return;
        }
    ...
    public void OnMinRandomRounds(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            GameModifiersUtils.PrintTitleToChat(player, $"Număr minim de modifiere setat la {minInput}");
            _minRandomRounds = result;
        }
        else
        {
            GameModifiersUtils.PrintTitleToChat(player, $"Eroare la setarea numărului minim de modifiere la {minInput}");
        }
    }
    ...
    public void OnMaxRandomRounds(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            GameModifiersUtils.PrintTitleToChat(player, $"Număr maxim de modifiere setat la {maxInput}");
            _maxRandomRounds = result;
        }
        else
        {
            GameModifiersUtils.PrintTitleToChat(player, $"Eroare la setarea numărului maxim de modifiere la {maxInput}");
        }
    }
    ...
    public void OnRandomRoundsReRoll(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            GameModifiersUtils.PrintTitleToChat(player, "Random rounds nu este activat! Nu se poate face re-roll.");
            return;
        }

        if (!RegisteredModifiers.Any())
        {
            GameModifiersUtils.PrintTitleToChat(player, "Nu există modifiere înregistrate! Nu se poate face re-roll.");
            return;
        }
    ...
    public HookResult OnRoundStart(EventRoundStart @event, GameEventInfo info)
    {
        if (RandomRoundsEnabled)
        {
            if (!RegisteredModifiers.Any())
            {
                GameModifiersUtils.PrintTitleToChatAll("Nu există modifiere înregistrate! Runda random sărită...");
            ...
            {
                GameModifiersUtils.PrintTitleToChatAll("Random rounds pornește după warmup...");
            }
            else
            {
                ApplyRandomRoundsForRound();
    ...
    public void ToggleRandomRounds()
    {
        ...
        GameModifiersUtils.PrintTitleToChatAll(RandomRoundsEnabled ? "Random rounds activat pentru runda următoare!" : "Random rounds dezactivat!");
    ...
    public void ApplyRandomRoundsForRound()
    {
        ...
        {
            LastActiveModifiers.AddRange(ActiveModifiers.Where(m => !LastActiveModifiers.Contains(m)));
            if (LastActiveModifiers.Count > _historySize * _maxRandomRounds)
            {
                LastActiveModifiers.RemoveRange(0, ActiveModifiers.Count);
            }
        }
        else
        {
            GameModifiersUtils.PrintTitleToChatAll($"Eroare la aplicarea modifierelor random! Runda sărită...");
        }
    }
    ...
    public void RemoveAllModifiers()
    {
        ...
        GameModifiersUtils.PrintTitleToChatAll("Se elimină modifierele:");
    ...
    private void ActivateModifiers(List<GameModifierBase> modifiers)
    {
        ...
        GameModifiersUtils.PrintTitleToChatAll("Se activează modifierele:");
